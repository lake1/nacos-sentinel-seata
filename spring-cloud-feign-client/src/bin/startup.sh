...
# config目录: 配置文件目录
# logs目录: 项目运行日志目录
# logs/spring-cloud-plus_startup.log: 记录启动日志
...
# 项目名称
APPLICATION="spring-cloud-feign-client-plus"
...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application background startup command: nohup java ${JAVA_OPT} -jar ${BASE_PATH}/lib/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} --logging.config=${CONFIG_DIR}logback-spring.xml > ${LOG_PATH} 2>&1 &\n"

JAVA_AGENT="-javaagent:${BASE_PATH}/agent/skywalking-agent.jar -Dskywalking_config=${BASE_PATH}/agent/config/agent.config -Dskywalking.agent.service_name=${APPLICATION}"
#======================================================================
# 执行启动命令：后台启动项目,并将日志输出到项目根目录下的logs文件夹下
#======================================================================
nohup java  ${JAVA_OPT} ${JAVA_AGENT} -jar ${BASE_PATH}/lib/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} --logging.config=${CONFIG_DIR}logback.xml > ${LOG_PATH} 2>&1 &
